...
// MapView component removed - inlined into GPSView
...
export default function FlowPlatformDemo() {
  ...
  // Only set mounted on client
  useEffect(() => {
    setMounted(true);
  }, []);

  // Separate useEffect for GPS updates (doesn't depend on currentTime)
  useEffect(() => {
  ...
  // Get current time only when needed for display
...
const MapView = memo(({ techPositions, selectedTech, onTechSelect }: {
  techPositions: {[key: number]: {x: number, y: number}},
  selectedTech: number | null,
  onTechSelect: (id: number) => void
}) => {
  return (
    <div 
      className="w-full h-[600px] rounded-lg overflow-hidden relative"
      ...
      <div className="lg:col-span-2">
        <div className="bg-slate-800/50 backdrop-blur rounded-xl border border-slate-700 p-6 h-[700px]">
          ...
          <MapView 
            ...
            onTechSelect={(id) => setSelectedTech(selectedTech === id ? null : id)}
          />
