...
// Stripe: if publishable key exists, load it. Otherwise keep a null promise that resolves to null.
...
export default function Home() {
  ...
  const handleCheckout = async () => {
    try {
      ...
      // Prefer using the URL directly (newer approach)
      if (url) {
        window.location.href = url
        return
      }

      // Fallback: try using Stripe.js redirectToCheckout (deprecated but may work with older versions)
      ...
      // For older Stripe.js versions that support redirectToCheckout
      if ('redirectToCheckout' in stripe) {
        (stripe as any).redirectToCheckout({ sessionId })
      } else {
        alert('Checkout redirect not supported. Please try again.')
      }
    } catch (err) {
